...
def refine_mask(
    mask: Optional[np.ndarray], image_path: str, search_radius: int = 10
) -> Optional[np.ndarray]:
    """
    ...
    Args:
        ...
        search_radius (int): Radius for A* search region (default: 10).
    ...
    if contour is None:
        # Return copy of input mask if no contour
        return (
            mask.copy()
            if len(mask.shape) == 2
            else cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)
        )
